...
- `.editorconfig` 是跨编辑器的基础格式基线。
- 文本文件默认使用 UTF-8 编码、LF 换行，并保留文件末尾换行。
- 默认使用空格缩进，不使用 tab。
- Java 使用 4 空格缩进。
- 前端、文档和配置文件使用 2 空格缩进。
- 提交前应消除尾随空格；`./scripts/validate-docs.sh` 会检查文档、模板和元数据的基础格式约束。
- `.gitattributes` 是 Git 文本规范基线，负责跨平台换行和二进制识别。
